	...
	//
	//
	// created by Bill Donner (billdonner@gmail.com) aish on 08-9-11.
	//  acsolu@gmail.com
...
@interface KORZipArchive (Private)
...
- (BOOL)addSkipBackupAttributeToItemAtURL:(NSURL *)URL;
{
	...
	//
	//[self addSkipBackupAttributeToItemAtPath:fullPath]; //donner
...
+ (BOOL)setSkipBackupAttributeToItemAtPath:(NSString *)path;
{
	const char* filePath = [path fileSystemRepresentation];
...
-(BOOL) isWierd:(NSString *)fullPath
{
	NSString *lastpath = [fullPath lastPathComponent];
	const char *last = [lastpath fileSystemRepresentation];
	
	return ((last[0]=='_')||(last[0]=='.'));
}
...
-(BOOL) UnzipFileTo:(NSString*) path overWrite:(BOOL) overwrite
{
    ...
    do{
        ...
        NSString* fullPath = [path stringByAppendingPathComponent:strPath];
			...
			// dont store wierd files with _ or . prefixes
		
		if ([self isWierd:fullPath])
		{
				//NSLog (@"skipping directory %@",[fullPath lastPathComponent] ); // set break here
			unzCloseCurrentFile( _unzFile );
        ...
        if( fp )
        {
            ...
            if( attr )
            {
				...
				// this should be blended into the above if possible
			
			
			[ self addSkipBackupAttributeToItemAtPath:fullPath]; // don't backup to icloud
